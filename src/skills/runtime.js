...
export class SkillRuntime {
    skills = new Map();
    triggerIndex = new Map(); // trigger phrase → skill name
    async loadFromDirectory(skillsDir) {
        try {
        ...
        if (frontmatterMatch) {
            ...
            for (const line of lines) {
                ...
                if (key === 'name')
                    metadata.name = value;
                if (key === 'description')
                    metadata.description = value.replace(/^>\s*/, '');
                if (key === 'triggers')
                    metadata.triggers = value.split(',').map(t => t.trim());
                if (key === 'requires')
                    metadata.requires = value.split(',').map(r => r.trim());
                if (key === 'version')
                    metadata.version = value;
                if (key === 'always_load')
                    metadata.alwaysLoad = value === 'true';
                if (key === 'auto_trigger')
                    metadata.autoTrigger = value !== 'false';
            }
        }
